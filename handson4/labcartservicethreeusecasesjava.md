# Lab Pack: Three Use Cases on One Repository (Java stack)
## E-commerce cart and checkout service

**Repository:** `cartsvc-java/`
**Duration:** 3 hours, or three separate 60-minute sessions
**Language:** Java 17, Maven, JUnit 5. No other dependencies.
...
```bash
tar -xzf cartsvc-java.tar.gz
cd cartsvc-java
mvn -q test                                                                     # 20 passed
mvn -q compile exec:java -Dexec.mainClass="com.shop.cart.fixtures.ConcurrentCheckoutFixture"
                                                                                 # both scenarios FAIL
```
...
| # | Defect | Where |
|---|---|---|
| 1 | Stock checked, then decremented later — not atomic | `CartService.java` — check loop and the `UPDATE` are separated by coupon validation, pricing and the risk call |
| 2 | Coupon usage limit checked, then incremented later — not atomic | `CouponService.java` `validate()` and `redeem()`, called at different points in `CartService.java` |
| 3 | GST computed on the **pre-discount** subtotal | `PricingService.java` `totals()` — `taxOn(sub)` before the discount is subtracted |
| 4 | Search query built with string interpolation into raw SQL | `CatalogService.java` `search()` |
| 5 | Email and full address written to application logs | `CartService.java` final `log.info(...)` |
| 6 | Price snapshotted at add-to-cart, never revalidated at checkout | `CartService.java` `addItem()` stores `unitPrice`; `checkout()` never rechecks |
...
`Connection` from the `DataSource`, so nothing spans a transaction. That
single observation explains defects 1 and 2 before either is
investigated.
...
Trace one call to:
  ...
  checkout(cartId, "user-1", "a@example.com", "12 MG Road", "WELCOME10")
...
# in CartService.java, move the stock decrement immediately after the
...
`CatalogService.java` and the email and address in the checkout log line.
...
nothing about composition or concurrency. `testTaxIsEighteenPercent` is
correct. `testDiscountIsTheCouponPercentage` is correct. The method that
...
**Java-specific trap worth flagging:** if the connection pool
(HikariCP or similar) is small, it can accidentally serialise the
concurrent checkouts onto too few connections and mask the race. Confirm
`maximumPoolSize` is comfortably above the fixture's shopper count before
assuming a quiet run means the bug is fixed.
