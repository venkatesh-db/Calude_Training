...
```bash
tar -xzf cartsvc-dotnet.tar.gz
cd cartsvc-dotnet
dotnet test                                            # 20 passed
dotnet run --project fixtures/ConcurrentCheckoutFixture # both scenarios FAIL
```
...
| # | Defect | Where |
|---|---|---|
| 1 | Stock checked, then decremented later — not atomic | `CartService.cs` — check loop and the `UPDATE` are separated by coupon validation, pricing and the risk call |
| 2 | Coupon usage limit checked, then incremented later — not atomic | `CouponService.cs` `Validate()` and `Redeem()`, called at different points in `CartService.cs` |
| 3 | GST computed on the **pre-discount** subtotal | `PricingService.cs` `Totals()` — `TaxOn(sub)` before the discount is subtracted |
| 4 | Search query built with string interpolation into raw SQL | `CatalogService.cs` `Search()` |
| 5 | Email and full address written to application logs | `CartService.cs` final `_logger.LogInformation(...)` |
| 6 | Price snapshotted at add-to-cart, never revalidated at checkout | `CartService.cs` `AddItem()` stores `UnitPrice`; `Checkout()` never rechecks |

**Defect 3 is the interesting one.** Every pricing method is correct in
isolation, and every unit test of them passes. Only the composition is
wrong. Calling `PricingService.Totals()` directly with a ₹1,000 cart and a
20% coupon gives tax of ₹180 on a discounted amount of ₹800 — the
customer is taxed ₹36 more than the goods they actually paid for.
...
boundary visible without naming it. Each method opens and closes its own
`SqlConnection` (or short-lived `DbContext`), so nothing spans a
transaction. That single observation explains defects 1 and 2 before
...
| Finding | Level |
|---|---|
| Six classes, each with a single responsibility | CONFIRMED |
| No foreign keys and no unique constraints anywhere in the schema | CONFIRMED |
| Every method opens its own connection; nothing spans a transaction | CONFIRMED |
| `CartItems.UnitPrice` stores a price snapshot | CONFIRMED |
| No test asserts a full checkout total; pricing is only tested per-method | CONFIRMED |
...
| Level | Looks like |
|---|---|
| Weak | Class summaries with no line references. Describes what the tests cover, not what they miss. |
| Adequate | Accurate map with addresses. Notes the per-method connections. Marks claims CONFIRMED or HYPOTHESIS. |
...
Trace one call to:
  ...
  Checkout(cartId, "user-1", "a@example.com", "12 MG Road", couponCode: "WELCOME10")
...
`AddItem()`. Nothing rechecks it at checkout. A cart sitting open for two
...
| Finding | Level |
|---|---|
| Stock read in a loop, then decremented much later on a separate connection | CONFIRMED |
| Between them: `CouponService.Validate`, `PricingService.Totals`, `RiskService.IsBlocked` | CONFIRMED |
| `RiskService.IsBlocked` is the expensive one and widens the window most | CONFIRMED |
...
| `UnitPrice` fixed at add-to-cart, never revalidated | CONFIRMED |
...
| Level | Looks like |
|---|---|
| Weak | Lists methods in order. No state, no interleaving analysis. |
| Adequate | Ordered trace with line numbers, correctly identifies both non-atomic windows. |
| Strong | Also names the three operations sitting inside the stock window, identifies the risk score as the dominant cost, and answers question 3 by connecting `AddItem` to `Checkout` across two methods. |
...
# in CartService.cs, move the stock decrement immediately after the
# check, before the coupon and risk calls
...
Review in this order:
  ...
  4. Blast radius — what else calls the changed methods?
...
findable in a review context — the interpolated-string SQL in
`CatalogService.cs` and the email and address in the checkout log line.
...
| `CatalogService.Search()` builds SQL with string interpolation (`$"..."`) | blocker |
...
nothing about composition or concurrency. `TaxIsEighteenPercent` is
correct. `DiscountIsTheCouponPercentage` is correct. The method that
combines them overcharges every discounted order. Unit tests verify
units; nobody wrote the test that verifies the whole.
...
**.NET-specific trap worth flagging:** if `CartService` resolves its
`DbContext` as scoped-per-request in a web host, running the fixture as a
bare console app instead can accidentally serialise the concurrent
checkouts onto a single, non-thread-safe `DbContext` instance and throw
rather than race. Confirm the fixture opens one connection per simulated
shopper, the same way the real API does, before assuming a quiet run
