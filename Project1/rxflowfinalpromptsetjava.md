# RxFlow — Final Prompt Set (Java stack)
...
| Role in original (Python) | Java equivalent used here |
|---|---|
| FastAPI + Pydantic v2 | Spring Boot 3 (Java 21) + Spring Web + Bean Validation |
| Plain service layer | Plain Java service layer (POJOs, no framework magic) |
| SQLAlchemy 2.x | Spring Data JPA / Hibernate |
| Alembic | Flyway (versioned, forward-only migrations — a closer match to how most Java shops actually work than a downgrade-per-revision tool) |
| Redis (cache + locks) | Spring Data Redis (Lettuce client) |
| Celery workers | Spring Kafka `@KafkaListener` consumers for event-driven work, plus Quartz Scheduler for the time-based surfacing/coating jobs — together covering what Celery covered |
| Kafka via Redpanda | unchanged — Kafka via Redpanda, client via Spring Kafka |
| pytest / pytest-asyncio / hypothesis / testcontainers | JUnit 5, Mockito, jqwik (property-based testing), Testcontainers for Java |
| ruff, mypy --strict, bandit, pip-audit | Checkstyle + SpotBugs (with FindSecBugs for the bandit role), OWASP Dependency-Check (the pip-audit role), compiler run with `-Xlint:all -Werror` |
| Docker Compose, OpenTelemetry | unchanged |
| Build tool | Maven (swap to Gradle if preferred — no defect design below depends on it) |
...
lab, not as the participants. Build RxFlow, a Java teaching
...
STACK
Spring Boot 3 (Java 21) + Spring Web + Bean Validation · plain Java
service layer · PostgreSQL with Spring Data JPA / Hibernate and Flyway
migrations · Redis via Spring Data Redis (Lettuce) for cache and locks ·
Spring Kafka consumers for event-driven background work, Quartz
Scheduler for time-based jobs (surfacing/coating scheduling) — together
filling the role Celery filled · Kafka via Redpanda · JUnit 5, Mockito,
jqwik, Testcontainers · Checkstyle, SpotBugs + FindSecBugs, OWASP
Dependency-Check, compiler with -Xlint:all -Werror · Maven · Docker
Compose · OpenTelemetry (Java agent).
...
Build the repo so each of these has a real, findable answer:
  ...
  2. Full request path for POST /orders — controller → DTO/request
     record → service → repository → Kafka event/consumer. Deep enough
     that tracing it takes effort, shallow enough to finish in 90
     minutes.
  ...
  4. Schema and migration state — Flyway with 4-6 versioned migrations,
     one of which has no rollback script provided anywhere in the repo.
  5. Redis and background-worker usage — Redis used as both cache and
     ...
     atomically. A background consumer or scheduled job with a retry
     policy that amplifies load on timeout.
  6. Outbound calls and timeouts — 3-4 external calls (via `RestClient`
     or a Feign client). At least one with no timeout configured at
     all. At least one with a retry policy that compounds with a
     caller's retry.
  7. Auth and authorization model — token auth (Spring Security) on
     most routes, and one endpoint (lab override) with no authorization
     check.
  8. Where PII reaches the logs — prescription values and patient
     ...
     call (SLF4J/Logback), one indirect via a DTO's generated
     `toString()` or a serialized log payload.
  9. Five highest-risk files — make risk genuinely uneven: one large
     ...
     rule), one concurrency-sensitive class, one with raw/native SQL.
...
Ten, surfacing across the week. Participants are not told what they are:
  ...
  4. Consumer/scheduler retry policy amplifies load ~3x on connector
     timeout
  ...
  9. Flyway migration with no rollback script and no documented manual
     rollback procedure
 10. Ad-hoc reporting query built with string concatenation — SQL
     injection
...
- Concurrency defects need realistic work inside the window — a pricing
  call, an external request. Never a Thread.sleep().
...
- mvn test (full suite) passes with all ten defects live
- Checkstyle, SpotBugs + FindSecBugs pass, and the build compiles clean
...
Evidence:
  - src/main/java/com/rxflow/orders/OrdersController.java
  - src/main/java/com/rxflow/orders/OrderService.java
  - src/main/java/com/rxflow/workers/OrderEventsConsumer.java
  - src/main/java/com/rxflow/db/entity/ (JPA entities),
...
2. RETRY OWNERSHIP — THIS IS HALF THE BUG
   ...
   Does the Kafka consumer or Spring Retry policy retry? Answer each
   separately, from the code, not from assumption. If more than one
   layer could be retrying the same request, say so explicitly — do
   not pick one and move on.
...
4. DATABASE TRANSACTION BOUNDARIES AND REDIS CONSISTENCY
   ...
   (@Transactional boundary or manual)? Is Redis used here at all —
   as a lock, a cache, a counter? Is any read-modify-write on Redis
   non-atomic?
...
- No float/double in money paths (BigDecimal only), no PII in logs, no
...
"Retries on the order-events consumer seem to be amplifying load
when a lab connector times out."
